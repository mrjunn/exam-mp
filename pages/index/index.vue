				...
				<template v-for="(item,i) in gridList">
					...
					>
						<view class="grid-item-box" style="background-color: #fff;">
							...
							<text v-if="!i" class="text" @click="goToExam({title:'软考初级题库', id:'635ce403bf704d0001b8c45a'})">软考初级题库</text>
							<text v-else class="text">{{item}}</text>
						</view>
					</uni-grid-item>
				</template>
			</uni-grid>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goToExam(d){
				uni.navigateTo({
					url: 'exam/paper?d='+JSON.stringify(d)
				});
			}
		}
	}
